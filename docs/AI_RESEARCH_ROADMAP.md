...
- AI research report JSON and Markdown responses include `ETag`, `Last-Modified`, and `Cache-Control: no-cache` freshness metadata; the dashboard JSON view surfaces the same report version fields, the endpoint honors `If-None-Match` and `If-Modified-Since` with `304 Not Modified`, and `HEAD /admin/ai/research-report` supports metadata-only checks.
