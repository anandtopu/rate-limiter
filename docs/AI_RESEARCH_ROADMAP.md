...
- AI research report JSON and Markdown responses include `ETag`, `Last-Modified`, `Cache-Control: no-cache`, `X-Report-Bytes`, and `X-Report-Lines` metadata; the dashboard JSON view surfaces the same report version fields, the endpoint honors `If-None-Match` and `If-Modified-Since` with `304 Not Modified`, `HEAD /admin/ai/research-report` supports metadata-only checks, and the dashboard uses cached validators on repeated report loads.
- Conditional Markdown report downloads preserve `Content-Disposition` filename metadata on `304 Not Modified` responses, matching the normal `200` download and metadata-only `HEAD` behavior.
